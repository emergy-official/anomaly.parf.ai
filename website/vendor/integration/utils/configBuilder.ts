...
const getEdgeImpulseConfig = (config: Config) => {
  const _default = {
    model_threshold: 10.0,
  };

  return merge({}, _default, config?.edgeimpulse ?? {}) as EdgeImpulseConfig;
};
...
export default (config: Config) => ({
  ...
  EDGEIMPULSE: getEdgeImpulseConfig(config),
});
